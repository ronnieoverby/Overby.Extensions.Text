...
namespace Overby.Extensions.Text
{
    public static class CsvParsingExtensions
    {
        ...
        public static IEnumerable<List<string>> ReadCsv(this TextReader reader, char delimiter = ',', char textQualifier = '"', List<string> record = null, bool trimValues = false)
        {
            ...
            {
                _ = fieldBuilder.Append(c);

                if (committed)
                    fieldLength = fieldBuilder.Length;
            }

            void FlushValue()
            {
                value = fieldBuilder.ToString(0, fieldLength);
                fieldLength = 0;
                fieldBuilder.Clear();
                trimming = trimValues;
            }
        ...
        /// <param name="textQualifier">The character that surrounds field text that may contain the delimiter or the text qualifier itself. For literal occurrences of this character within the field data, the character should be doubled.</param>
        /// <returns>An enumerable of string arrays.</returns>
        public static IEnumerable<List<string>> ReadCsv2(this TextReader reader, char delimiter = ',', char textQualifier = '"', List<string> record = null, bool trimValues = false)
        {
            if (reader == null) throw new ArgumentNullException(nameof(reader));

            var buffers = new List<IMemoryOwner<char>>(); // todo dispose all
            ...
            evaluate characters in buffer

            when field value is finally terminated,
                string.create from buffer(s)
                    and store in record list

            if buffer runs out,
                we could rent another buffer
                    filling it in until field terminates
                        then string create over all rented buffers
                            

            finally,
                get rid of rented buffers
